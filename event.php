<?php
    ...
    $removeReview_id = isset($_GET["review_id"]) ? $_GET["review_id"] : null;
	...
	if($eventAttendReview !== null && $rate !== null && $reviewUser_id !== null & $eventAttendImage !== null)
	{
		...
		if(in_array($imageFileType, $allowed) && $eventAttendImage["size"] < 1048576 ){ 
            ...
            else
            {
					...
					$query = "SELECT review_id FROM tbreviews WHERE user_id = '$reviewUser_id' AND event_id ='$event_id'";
                ...
                <strong>Good Stuff! </strong>Review Created!
                </div>";
    ...
    if($removeReview_id)
    {
        $bail = "DELETE FROM tbreviews WHERE review_id = '$removeReview_id'";
        $bailed = mysqli_query($mysqli, $bail) == TRUE;

        $bailImg = "DELETE FROM tbreviewgallery WHERE review_id = '$removeReview_id'";
        $bailed = mysqli_query($mysqli, $bailImg) == TRUE;

        echo "<div class='alert alert-success ' role='alert'>
            <strong>Good Stuff! </strong>Review Deleted!
            </div>";
    }
...
<body>
	...
	<div class="container">
        ...
        if($eventStatus == 'active')
        {
            ...
            {
                echo 	"<p class='lead'><a id='otherevent' href='profile.php?user_id=". $eventUserID ."'>". $eventUserName . " " . $eventUserSurname . "</a> created this event</p>";
                if($user_type == "admin")
                {
                    echo "<a class='btn' id='adminbutton' href='editevent.php?event_id=". $event_id ."' role='button'>Edit Event</a><a class='btn' id='adminbutton' href='deleteevent.php?event_id=". $event_id ."'>Delete Event</a>";
                }
            }
        }
        else
        {
            echo 	"<p class='lead'>This event has been deleted</p>";
        }
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-sm-12">   
                ...
                {
                    echo "<div class='card-body'>
                    <h5 class='card-title'>You cannot edit or attend a deleted event. The reason this event has a page is because users have attended this event.</h5>
                    </div>";
                }
                ?>
                </div>
        ...
        <div class="row">
                        ...
                        <?php 
                             ...
                             while($stars = mysqli_fetch_array($resStars)){
                                ...
                                echo "<p class='card-text'>This event has an average rating of ". floor($stars['AVG(stars)']);
                                    ...
                                    for($i = 0; $i <floor($stars['AVG(stars)']); $i++)
                                    {
                                        echo " ★ ";
                                    }
                                echo "</p>";}
            ...
            <div class="col-lg-6 col-sm-6">
                <div class="card border-light shadow mb-5 rounded">
                    ...
                    <?php if($eventStatus == 'active')
                            {
                        echo "<h5 class='card-title'>Add this event to a list?  <button type='submit' class='btn btn-dark' id='addToList'>Add to List <i class='fa-solid fa-plus'></i></button></h5>";
                            }
                            else
                            {
                                echo "<h5 class='card-title'>Want to recover this event? Contact your system admin for assistance.</h5>";
                            }
                            ?>
       ...
       <div class="row scroll" style="height: 40rem">
            ...
            <?php 
                ...
                $resRev= $mysqli->query($queryRev);
                while($revs = mysqli_fetch_array($resRev)){
                    
                    if($count == 3)
                    {
                        $count = 0;
                    }
                    $userRev = $revs['user_id'];
                    $rev_id = $revs['review_id'];
                    ...
                    if($uvs = mysqli_fetch_array($u))
                    {
                        ...
                        $query2 = "SELECT image_name FROM tbreviewgallery WHERE review_id ='$rev_id'";
						$img = $mysqli->query($query2);
						if($i = mysqli_fetch_array($img))
                        {
                            $image = $i['image_name'];
                            ...
                            <div class='card' id='". $cardColours[$count] ."'>
                                ...
                                <div class='card-body'>";
                                if($user_type == "admin" && $user_id != $userRev)
                                {
                                    echo "<h5 class='card-title'><a id='revLink' href='profile.php?user_id=". $userRev ."'>". $uvs['name'] . " " . $uvs['surname'] . " </a> <a class='btn' id='adminbutton' href='event.php?event_id=".$event_id ."&review_id=". $rev_id ."' role='button'>Remove</a><br>". $revs['stars'] . " stars ";
                                }
                                else if($eventStatus == "deleted" && $user_id == $userRev ) 
                                {   
                                    echo "<h5 class='card-title'><a id='revLink' href='profile.php?user_id=". $userRev ."'>". $uvs['name'] . " " . $uvs['surname'] . " </a> <a class='btn btn-dark' href='event.php?event_id=".$event_id ."&review_id=". $rev_id ."' role='button'>Remove</a><br>". $revs['stars'] . " stars ";
                                }
                                else if($eventUser == $user_id && $user_id != $userRev) 
                                {   
                                    echo "<h5 class='card-title'><a id='revLink' href='profile.php?user_id=". $userRev ."'>". $uvs['name'] . " " . $uvs['surname'] . " </a> <a class='btn btn-dark' href='event.php?event_id=".$event_id ."&review_id=". $rev_id ."' role='button'>Remove</a><br>". $revs['stars'] . " stars ";
                                }
                                else
                                {
                                    echo "<h5 class='card-title'><a id='revLink' href='profile.php?user_id=". $userRev ."'>". $uvs['name'] . " " . $uvs['surname'] . " </a> <br>". $revs['stars'] . " stars ";
                                }
                                    
                                    for($i = 0; $i < $revs['stars']; $i++)
                                    {
                                        echo "★";
                                    }
                                    echo " </h5>
                                    <p class='card-text'>". $revs['comment'] ."</p>
                                </div>
